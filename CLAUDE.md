...
- **Nothing that reads mapping keys from `chrome.storage.local` may run before the sync→local migration finishes** — that migration is what puts them there, so a reader that starts first sees nothing, and only on the single startup where the move actually happens. The migration is one single-flight run owned by `storageReadiness` (`src/api/storageReadiness.js`): the startup prerequisite starts it, and anything touching `drive_location_mappings` or the `summary_*` keys — reads *and* writes — must `await storageReadiness.ensure()` first (see `driveMappings.loadMappings` and `saveToStorage`) rather than relying on mount order; Vue mounts children before the parent's `onMounted`, so mount order guarantees nothing. Writers wait for the mirror reason: the migration's copy is the last write to win, so a change persisted mid-copy is silently reverted to legacy sync data.
